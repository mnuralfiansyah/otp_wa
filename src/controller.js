...
async function sendSocialVoucherLink(req, res) {
  try {
    ...
    await global.WA_CLIENT.sendText(`${phone}@.us`, msg)
      .then((result) => {
        return res.status(200).send({ result: result });
      })
      .catch((err) => {
...
async function sendImageToGroup(req, res) {
  try {
    ...
    await global.WA_CLIENT.sendImage(`${phone}@g.us`, url_image, 'unnamed.jpg', message)
      .then((result) => {
        return res.status(200).send({ result: result });
      })
      .catch((err) => {
...
async function getAllChat(req, res) {
  try {
    ...
    await global.WA_CLIENT.getAllChatsGroups()
      .then((result) => {
        return res.status(200).send({ result: result });
      })
      .catch((err) => {
